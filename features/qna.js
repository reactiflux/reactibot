const qna = {
	...
	flush: (msg) => {
		qna.prevMessage.forEach(oldId => msg.channel.fetchMessage(oldId).then(msg => msg.delete()));
		qna.prevMessage = [];
	},
	
	prompt: (msg) => {
		if (qna.counter < 3) {
			msg.channel.send(`:robot: Please ask your question now. Our guest has ${qna.counterAsWord(qna.counter)} questions queued. As a reminder - we limit the queue to 3 questions at a time. Please remember to start your question with [Q&A] - thank you!`).then(msg => {
				qna.flush(msg);
				qna.prevMessage.push(msg.id);
			});
		} else {
			msg.channel.send(`:robot: Please stop your questions for now. Our guest has ${qna.counterAsWord(qna.counter)} questions queued.`).then(msg => {
				qna.flush(msg);
				qna.prevMessage.push(msg.id);
			})
		}		
	...
	triggers: {
		...
		'❌': (msg, user) => {
			msg.author.send(`Hello there! Your message has been removed from the Question and Answers channel by a moderator.
