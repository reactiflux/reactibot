...
const jobs = {
    ...
    user, 
		...
		jobs.tags.forEach(tag => {
			if(msg.content.toLowerCase().indexOf(`[${tag}]`) !== -1) hasTags = true;
		});
		if(!hasTags && msg.mentions.members.array().length === 0) {
			...
			msg.author.send(`Hello there! It looks like you just posted a message to the #jobs channel on our server.
